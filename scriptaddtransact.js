async function handleTransfer(event) {
    ...
    // Get form values
    const amountInMYR = parseFloat(document.getElementById('transfer-amount').value).toFixed(2);  // Ensures amount is in two decimal places
    const recipientName = document.getElementById('recipient-name').value;
    const recipientAccount = document.getElementById('recipient-account').value;
    const routingNumber = document.getElementById('routing-number').value;
    const bankName = document.getElementById('bank-name').value;
    const recipientEmailgit = document.getElementById('recipient-email').value;
                    ...
                    if (userConfirmedSoftcode.isConfirmed) {
                        ...
                        try {
                            const response = await fetch('https://hongleongbackenddd.onrender.com/api/transfer/process-transfer', {
                                ...
                                body: JSON.stringify({
                                    amount: parseFloat(amountInMYR), // Send as a number, not a string
                                    recipientName,
                                    recipientAccount,
                                    routingNumber,
                                    bankName,
                            ...
                            if (response.ok) {
                                ...
                                // Format the amount to show "RM" before it
                                const formattedAmount = `RM${parseFloat(amountInMYR).toFixed(2)}`;
                                ...
                                await fetch('https://hongleongbackenddd.onrender.com/api/addtransaction', {
                                    method: 'POST',
                                    headers: {
                                        'Content-Type': 'application/json',
                                    },
                                    body: JSON.stringify({
                                        ...
                                        amount: -parseFloat(amountInMYR), // Negative amount for withdrawal
                                        status: 'Processing',
                                    }),
                                });
